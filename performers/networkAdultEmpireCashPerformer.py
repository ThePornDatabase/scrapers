...
class networkAdultEmpireCashPerformerSpider(BasePerformerScraper):
    ...
    def get_ethnicity(self, response):
        if 'ethnicity' in self.selector_map:
            ...
            if ethnicity:
                if ":" in ethnicity:
                    ethnicity = re.search('Ethnicity:\ (.*)', ethnicity).group(1)
                    if ethnicity:
    ...
    def get_eyecolor(self, response):
        if 'eyecolor' in self.selector_map:
            ...
            if eyecolor:
                if ":" in eyecolor:
                    eyecolor = re.search('Eyes:\ (.*)', eyecolor).group(1)
                    if eyecolor:
    ...
    def get_haircolor(self, response):
        if 'haircolor' in self.selector_map:
            ...
            if haircolor:
                if ":" in haircolor:
                    haircolor = re.search('Hair:\ (.*)', haircolor).group(1)
                    if haircolor:
            ...
            if height:
                if ":" in height:
                    ...
                    if height:
                        ...
                        str_height = re.search('(\d).*(\d{1,2})', str_height)
    ...
    def get_weight(self, response):
        if 'weight' in self.selector_map:
            ...
            if weight:
                if ":" in weight:
                    weight = re.search('Weight:\ (.*)', weight).group(1)
                    if weight:
                        return weight.strip()
        return ''                   
    ...
    def get_measurements(self, response):
        if 'measurements' in self.selector_map:
            ...
            if measurements:
                if ":" in measurements:
                    measurements = re.search('Meas:\ (.*)', measurements).group(1)
                    if measurements:
    ...
    def get_cupsize(self, response):
        if 'measurements' in self.selector_map:
            ...
            if measurements:
                if "-" in measurements:
                    cupsize = re.search('Meas:\ (.*?)-.*', measurements).group(1)
                    if cupsize:
