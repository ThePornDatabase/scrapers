...
class SiteAD4XPerformerSpider(BasePerformerScraper):
    name = 'AD4XPerformer'
    site = 'AD4X'
    parent = 'AD4X'
    network = 'Radical Entertainment'

    start_url = 'https://tour.ad4x.com'

    # Used for the one model in the feed with an empty gender field
    default_gender = 'Female'

    selector_map = {
        'external_id': r'',
        # An empty gender= is required: without it the endpoint returns females
        # only (341 of 379).  No order_by - adding one drops 5 models that have
        # no publish_date.
        'pagination': '/_next/data/<buildID>/en/models.json?page=%s&gender=',
    }
    ...
    def get_performers(self, response):
        ...
        for performer in jsondata:
            ...
            # The only free-text field the feed carries; usually an empty <p><br></p>
            item['bio'] = self.clean_bio(performer.get('details'))

            item['url'] = f"{self.start_url}/en/models/{performer['slug']}"
            item['site'] = self.site
