...
class SiteThirdWorldMediaMovieSpider(BaseSceneScraper):
    name = 'MovieThirdWorldMedia'
    network = 'Third World Media'
    parent = 'Third World Media'
    site = 'Third World Media'
    ...
    selector_map = {
        ...
        'external_id': 'scene/(\\d+)',
        'trailer': '',
        'pagination': '/third-world-media-streaming-videos.html?page=%s&hybridview=member'
    }
    ...
    def parse_movie(self, response):
        meta = response.meta
        sceneurls = response.xpath('//div[contains(@class,"item-grid-scene")]//article/div[1]/a/@href').getall()
        sceneurls = list(filter(lambda x: len(x) > 0, sceneurls))
        if len(sceneurls) > 1:
            item = SceneItem()
            item['network'] = 'Third World Media'
            item['parent'] = 'Third World Media'
            item['site'] = 'Third World Media'
            ...
            if submitmovie:
                ...
                director = response.xpath('//div[@class="director"]/span/following-sibling::text()[1]')
                if director:
