...
class SiteNicoNiceSpider(BaseSceneScraper):
    name = 'NicoNice'
    network = "Nico Nice"
    parent = "Nico Nice"
    site = "Nico Nice"
    ...
    def get_scenes(self, response):
        ...
        for scene in jsondata:
            ...
            item['image'] = ''
            item['image_blob'] = ''
            link = f"https://nico-nice.com/index.php/wp-json/wp/v2/media?parent={item['id']}"
            ...
            item['performers'] = []
            link = f"https://nico-nice.com/index.php/wp-json/wp/v2/video-cast?post={item['id']}"
            req = requests.get(link, headers=reqheaders, timeout=10)
            if req and len(req.text) > 5:
                ...
                for performer in performerlist:
                    ...
                    elif performer['name'].lower() == "master nico":
                        item['performers'].append("Nico Nice")
                    else:
                        item['performers'].append(self.cleanup_title(performer['name']))
            if "Nico Nice" not in item['performers']:
                item['performers'].append("Nico Nice")
            ...
            tags = []
            link = f"https://nico-nice.com/index.php/wp-json/wp/v2/video-genres?post={item['id']}"
            req = requests.get(link, headers=reqheaders, timeout=10)
            if req and len(req.text) > 5:
                taglist = []
            ...
            item['trailer'] = ""
            item['site'] = "Nico Nice"
            item['parent'] = "Nico Nice"
            item['network'] = "Nico Nice"
