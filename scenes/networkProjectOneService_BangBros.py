...
class ProjectOneServiceBangBrosSpider(BaseSceneScraper):
    name = 'ProjectOneServiceBangBros'
    network = 'mindgeek'
    ...
    def get_scenes(self, response):
        ...
        for scene in response.json()['result']:
            item = SceneItem()
            if scene['brand'].lower().strip() == "bangbros":
                ...
                item['url'] = f"https://www.{siteurl}.com/video/{scene['id']}/{slugify(item['title'])}"
