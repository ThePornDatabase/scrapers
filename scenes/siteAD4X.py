...
class AD4XSpider(BaseSceneScraper):
    ...
    selector_map = {
        'external_id': r'',
        'pagination': '/_next/data/<buildID>/en/videos.json?page=%s&order_by=publish_date&sort_by=desc',
        'type': 'Scene',
    }

    # The scene JSON gives each model as {name, slug, thumb} with no gender, so the
    # model list is pulled once up front to build a slug -> gender map.  An empty
    # gender= is required: without it the endpoint returns females only (341 of 379).
    models_url = '/_next/data/%s/en/models.json?page=1&per_page=1000&gender='
    ...
    def get_scenes(self, response):
        ...
        for scene in jsondata:
            ...
            if "seconds_duration" in scene:
                ...
                item['duration'] = None
            item['id'] = scene['id']
            item['url'] = f"{self.start_url}/en/videos/{scene['slug']}"
            item['site'] = self.site
