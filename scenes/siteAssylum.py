...
class AssylumSpider(BaseSceneScraper):
    ...
    def get_image(self, response):
        ...
        if not image:
            image = response.xpath('//div[@class="mainpic"]/img/@src').get().strip()
    ...
    def get_trailer(self, response):
        image = response.xpath('//script[contains(text(),"mp4")]').get().strip()
        image = self.get_domain(response) + re.search('src:\s+\'(.*)\'', image).group(1).strip()
        return image
