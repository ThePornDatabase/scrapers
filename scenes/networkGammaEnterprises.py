...
class GammaEnterprisesSpider(BaseSceneScraper):
    ...
    def get_image(self, response):
        ...
        matches = ['bskow' ,  'footsiebabes' ,  'burningangel' ,  'devilsgangbangs' ,  'hothouse', 'tsfactor', 'lewood', 'maledigital','squirtingorgies', 'tittycreampies']
    ...
    def get_date(self, response):
        matches = ['christophclarkonline' ,  'gapingangels' ,  'jakemalone' ,  'joeysilvera', 'lewood','nachovidalhardcore', 'povblowjobs', 'tittycreampies']
        if any(x in response.url for x in matches):
    ...
    def get_performers(self, response):
        matches = ['christophclarkonline' ,  'gapingangels' ,  'jakemalone' ,  'joeysilvera', 'lewood', 'povblowjobs']
        if any(x in response.url for x in matches):
            performer = response.xpath('//meta[@name="description"]/@content').get()
            if "starring" in performer.lower():
                performer = re.search('starring\ (.*?)\ in', performer).group(1)
                if "," in performer:
        ...
        if any(x in response.url for x in matches):
            ...
            if "starring" in tag.lower():
                if " last updated " in tag:
                    tag = re.search('\ in\ (.*?)\ last\ updated', tag.lower()).group(1)
                else:
    ...
    def get_id(self, response):
        if "lexingtonsteele" in response.url:
            search = re.search('\/video\/(\d*)\/', response.url, re.IGNORECASE)
        else:
