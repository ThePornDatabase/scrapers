...
class SiteHookupHotshotByPerformerSpider(BaseSceneScraper):
    name = 'HookupHotshotByPerformer'
    ...
    def get_scenes(self, response):
        models = response.xpath('//div[contains(@class, "item-portrait")]/a/@href').getall()
        for model in models:
            if "/models/" in model:
                yield scrapy.Request(url=self.format_link(response, model), callback=self.get_model_scenes)
