...
class BangBrosSpider(BaseSceneScraper):
    ...
    def get_scenes(self, response):
        ...
        for scene in scenes:
            ...
            if scene.xpath("//span[contains(@class, 'thmb_mr_cmn')][2]//span[@class='faTxt']/text()") is not None:
                date = scene.xpath("//span[contains(@class, 'thmb_mr_cmn')][2]//span[@class='faTxt']/text()").get()
    ...
    def get_trailer(self, response):
        return 'https:' + self.process_xpath(response, self.get_selector_map('trailer')).get()
