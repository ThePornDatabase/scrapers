...
class SiteLucidFlixSpider(BaseSceneScraper):
    name = 'LucidFlix'

    start_url = 'https://lucidflix.com/'

    selector_map = {
        'external_id': r'',
        'pagination': '/_next/data/<buildID>/episodes.json?page=%s&order_by=publish_date&sort_by=desc',
    }

    def start_requests(self):
        meta = {}
        meta['page'] = self.page
        yield scrapy.Request('https://lucidflix.com/', callback=self.start_requests_2, meta=meta, headers=self.headers, cookies=self.cookies)
    ...
    def get_scenes(self, response):
        ...
        for scene in jsondata:
            ...
            item['url'] = f"https://lucidflix.com/episodes/{scene['slug']}"

            item['tags'] = scene['tags']

            item['duration'] = scene['seconds_duration']
            item['site'] = 'LucidFlix'
            item['parent'] = 'LucidFlix'
            item['network'] = 'LucidFlix'
            item['performers'] = []
