...
class SiteJacquieEtMichelTVDateWorkSpider(BaseSceneScraper):
    name = 'JacquieDateWork'
    ...
    def get_scenes(self, indexdata):
        ...
        headers = self.headers
        headers['Content-Type'] = 'application/json'
        scenes = response.xpath('//div[@class="video-list" and not(./a/h2)]//div[@class="gl__col video-item"]')
        for scene in scenes:
            if "page" not in response.url:
                scenedate = scene.xpath('.//p[contains(@class, "info")]/text()')
                if scenedate:
            ...
            scene = scene.xpath('.//a[@class="video-item__thumb"]/@href').get()
            print (f"Scene0: {scene}")
            if scene:
                print (f"Scene1: {scene}")
                if re.search(self.get_selector_map('external_id'), scene):
                    print (f"Scene2: {scene}")
                    my_data = {'cmd': 'request.get', 'maxTimeout': 60000, 'session': 'jacquie', 'url': "https://www.jacquieetmicheltv.net" + scene}
                    yield scrapy.Request(self.flare_address, method='POST', callback=self.parse_scene, body=json.dumps(my_data), headers=headers, cookies=self.cookies, meta=meta)
