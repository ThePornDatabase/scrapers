...
class SiteRedXXXSpider(BaseSceneScraper):
    name = 'RedXXX'

    start_urls = [
        'https://tour.red-xxx.com'
    ]
    ...
    def get_scenes(self, response):
        # ~ print(response.text)
        scenes = response.xpath('//div[contains(@class, "tit-main")]//h2/a/@href').getall()
        for scene in scenes:
    ...
    def parse_scene(self, response):
        ...
        if jsondata:
            ...
            item['trailer'] = ""
            item['url'] = f"https://tour.red-xxx.com/scenes/{jsondata['slug']}"
            item['type'] = 'Scene'
