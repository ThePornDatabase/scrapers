...
class SiteSexyHippiesSpider(BaseSceneScraper):
    name = 'SexyHippies'

    start_urls = [
        'https://sexyhippies.mymember.site',
    ]
    ...
    def get_scenes(self, response):
        ...
        for scene in scenes:
            ...
            item['site'] = "Sexy Hippies"
            item['parent'] = "Sexy Hippies"
            item['network'] = "Sexy Hippies"
            item['url'] = f"https://sexyhippies.com/videos/{item['id']}"
            meta['item'] = item.copy()
            yield scrapy.Request(f"https://sexyhippies.mymember.site/api/videos/{item['id']}", callback=self.get_scene_details, meta=meta)
    ...
    def get_scene_details(self, response):
        ...
        if "casts" in scene and scene['casts']:
            for cast in scene['casts']:
                ...
                if "sexy hippies" not in name.lower():
