...
class MontysPOVSpider(BaseSceneScraper):
    ...
    def get_scenes(self, response):
        ...
        for sceneresponse in sceneresponses:
            ...
            if not date:
                date = datetime.now()
            item['date'] = dateparser.parse(date.strip(), date_formats=['%d-%m-%Y']).isoformat()
            
            # Performer
            performer = sceneresponse.xpath('./div[@class="meta"]/div/a[contains(@class,vidLinkName)]/text()').getall()
            if not performer:
                performer = []
            item['performers'] = list(map(lambda x: x.strip(), performer))
            
            # Tags
            tags = sceneresponse.xpath('./div[@class="meta"]/div/div[@class="subListCats"]/a/text()').getall()
            if not tags:
                tags = []
