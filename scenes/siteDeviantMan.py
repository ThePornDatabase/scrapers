...
class SiteDeviantManSpider(BaseSceneScraper):
    name = 'DeviantMan'

    start_urls = [
        'https://www.deviantman.com',
    ]
    ...
    def start_requests(self):
        ...
        product_tag = requests.get("https://deviantman.com/wp-json/wp/v2/product_tag?per_page=100", headers=reqheaders, timeout=10)
        if product_tag and len(product_tag.text) > 5:
            meta['product_tag'] = json.loads(product_tag.text)
        else:
            meta['product_tag'] = []

        product_cat = requests.get("https://deviantman.com/wp-json/wp/v2/product_cat?per_page=100", headers=reqheaders, timeout=10)
    ...
    def get_scenes(self, response):
        ...
        for scene in jsondata:
            ...
            item['trailer'] = ''
            item['site'] = "Deviant Man"
            item['parent'] = "Deviant Man"
            item['network'] = "Deviant Man"
            item['type'] = 'Scene'
            meta['item'] = item
