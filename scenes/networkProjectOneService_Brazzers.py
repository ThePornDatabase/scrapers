...
class ProjectOneServiceBrazzersSpider(BaseSceneScraper):
    name = 'ProjectOneServiceBrazzers'
    network = 'mindgeek'

    # For some reason Brazzers isn't working any longer in the scheduled tasks when it's with anything else...
    ...
    def get_scenes(self, response):
        ...
        for scene in response.json()['result']:
            item = SceneItem()
            if scene['brand'].strip().lower() == "brazzers":
                ...
                item['url'] = f"https://www.brazzers.com/video/{scene['id']}/{slugify(item['title'])}"
