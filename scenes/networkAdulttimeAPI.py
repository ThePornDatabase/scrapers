...
def match_site(argument):
    match = {
        ...
        'gloryholesecreta': 'Gloryhole Secrets',
...
class AdultTimeAPISpider(BaseSceneScraper):
    ...
    def get_next_page_url(self, base, page):
        matches = ['isthisreal', 'touchmywife', 'zerotolerance', 'povthis', 'tabooheat', 'puretaboo', 'pridestudios', 'prettydirty']
    ...
    def get_scenes(self, response):
        ...
        for scene in response.json()['results'][0]['hits']:
            ...
            if 'devilsfilm' in referrerurl:
                ...
                item['url'] = self.format_url(response.meta['url'], '/en/video/' + scene['sitename'] + '/' + scene['url_title'] + '/' + str(scene['clip_id']))
            if 'dogfartnetwork' in referrerurl:
            ...
            if ('dogfartnetwork' in item['network'] and item['date'] < "2023-06-20"):
                donotyield = 1
    ...
    def call_algolia(self, page, token, referrer):
        ...
        if 'dogfartnetwork' in referrer:
            jbody = '{"requests":[{"indexName":"all_scenes_latest_desc","params":"query=&hitsPerPage=24&page=' + str(page) + '&filters=(NOT%20sitename%3A\'dfxtrapartners\')&clickAnalytics=true&facets=%5B%5D&tagFilters=&facetFilters=%5B%22categories.name%3A-Behind%20The%20Scene%22%2C%22upcoming%3A0%22%5D"}]}'
