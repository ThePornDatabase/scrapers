...
class SiteDripDropSpider(BaseSceneScraper):
    ...
    start_url = 'https://dripdropprod.net/'
    ...
    def get_scenes(self, response):
        ...
        for scene in jsondata:
            ...
            item['url'] = f"https://dripdropprod.net/scenes/{scene['slug']}"

            tags = scene['tags']
            for tag in tags:
                if "drip" not in tag.lower():
                    item['tags'].append(string.capwords(tag))

            item['duration'] = scene['seconds_duration']
            item['site'] = 'DripDrop'
            item['parent'] = 'DripDrop'
            item['network'] = 'DripDrop'
            item['performers'] = []
            for model in scene['models_slugs']:
                item['performers'].append(model['name'])
                if model['name'] in item['tags']:
                    item['tags'].remove(model['name'])
            if "DripDrop" not in item['performers']:
                item['performers'].append("DripDrop")
