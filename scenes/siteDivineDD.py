...
class SiteDivineDDSpider(BaseSceneScraper):
    name = 'DivineDD'

    start_url = 'https://divine-dd.com'
    ...
    def start_requests(self):
        meta = {}
        meta['page'] = self.page
        yield scrapy.Request('https://divine-dd.com/', callback=self.start_requests_2, meta=meta, headers=self.headers, cookies=self.cookies)
    ...
    def get_scenes(self, response):
        ...
        for scene in jsondata:
            ...
            item['url'] = f"https://divine-dd.com/videos/{scene['slug']}"
            item['tags'] = []  # Tags only return breast size
            # ~ if "tags" in scene:
            # ~ item['tags'] = scene['tags']
            item['duration'] = scene['seconds_duration']
            item['site'] = 'DivineDD'
            item['parent'] = 'DivineDD'
            item['network'] = 'DivineDD'
            item['performers'] = []
