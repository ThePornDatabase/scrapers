...
class PrivateClassicsMoviesSpider(BaseSceneScraper):
    name = 'PrivateClassicsMovies'
    network = "Private"
    ...
    def parse_movie(self, response):
        meta = response.meta
        scenes = response.xpath('//h3/a[contains(text(), "Scenes")]/@href').get()
        if len(scenes) > 1:
            item = SceneItem()
            item['title'] = self.cleanup_title(response.xpath('//div[@class="container"]/div[contains(@class, "content-text")]//h1/text()').get().strip())
            scenedate = response.xpath('//div[@class="container"]/div[contains(@class, "content-text")]//p[contains(@class, "release")]//text()').getall()
            ...
            # ~ item['image_blob'] = ''
            director = response.xpath('//div[@class="container"]//h3[contains(@class, "directe")]/text()')
            if director:
            ...
            if director:
                ...
                item['director'] = ''
            item['performers'] = response.xpath('//div[@class="container"]/div[contains(@class, "content-text")]//a[contains(@href, "/pornstar")]/text()').getall()
    ...
    def parse_scene(self, response):
        ...
        movie = meta['movie']
        item = SceneItem()
        item['title'] = self.cleanup_title(response.xpath('//div[contains(@class, "user-tools")]/preceding-sibling::h1/text()').get().strip())
        item['date'] = movie['date']
        description = response.xpath('//div[contains(@class, "user-tools")]/preceding-sibling::p/text()')
        if description:
            item['description'] = description.get().strip()
        else:
            item['description'] = ""
        ...
        item['performers'] = response.xpath('//div[contains(@class, "user-tools")]/..//a[contains(@href, "/pornstar")]/text()').getall()
